...
test("les refus qui n'ont rien à voir gardent leur message d'origine", () => {
  ...
  assert.match(messageEchecIdentifiants("over_request_rate_limit"), /Trop d'essais/);
  assert.match(messageEchecEnvoi("over_request_rate_limit"), /Trop d'essais/);
  assert.match(messageEchecEnvoi("email_address_invalid"), /forme valide/);
});

test("un jeton refusé n'est SURTOUT pas rangé parmi les refus qu'on tait", () => {
  // C'est le piège propre au Control Center. Cette page tait trois
  // refus, parce qu'ils diraient si l'adresse appartient à un
  // administrateur. Y ajouter celui du CAPTCHA serait une faute de
  // deux façons :
  //
  //   1. l'écran afficherait le panneau calme « un code vient d'être
  //      envoyé, s'il y a lieu » alors que RIEN n'est parti — même
  //      pour un collègue dont le compte existe. Il attendrait un
  //      courriel qui ne viendrait jamais.
  //   2. et ça ne cacherait rien : le contrôle du jeton s'exécute
  //      AVANT que le serveur ne cherche le compte, sa réponse est
  //      donc rigoureusement la même pour tout le monde.
  assert.equal(envoiDoitResterMuet("captcha_failed"), false);
  // Les trois vrais refus tus n'ont pas bougé.
...
test("le widget est sombre, comme le reste de la console", () => {
  // `globals.css` pose `html { color-scheme: dark }` : cette console
  // est sombre quoi qu'en dise le système. Le réglage « auto » de
  // Turnstile, lui, suit la préférence du système — il poserait donc
  // une tache blanche au bas de la carte pour tout administrateur dont
  // l'ordinateur est en mode clair.
